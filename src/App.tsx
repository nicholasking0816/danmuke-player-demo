...
export class App extends React.Component<any, any, any> {
  ...
  gDanmukes() {
    ...
    let randomCar = "发哈开朗说得好像都我破碎机接口都发给你分工会的体育部";
    let randomCarLen = randomCar.length;
    for (let i = 0; i < 200; i++) {
      let randomLen = Math.round(10 * Math.random());
      let start = Math.floor(Math.random() * (randomCarLen - randomLen));
      danmukeList.push({
        content: "夏目小天使yyds" + randomCar.slice(start, start + randomLen),
        id: uuidV4(),
        // layer: 'AdvanceLayer',
  ...
  componentDidMount() {
    ...
    this.player.addDmkLayer(
      new DmkLayer(AdvanceDmkTrack, {
        priority: 10,
        trackCount: 1,
      }),
      "AdvanceLayer"
    ...
    return (
      <div className="danmuke-player-demo">
        <div className="btn-paly-advance-wrapper">
          <button
            className="btn-paly-advance"
            disabled={this.state.disabled}
            onClick={this.palyAdvance.bind(this)}
          >
            播放高级弹幕
          </button>
        </div>
        ...
        <div className="send-danmuke">
          <div className="normal-danmuke">
            ...
            <button onClick={this.sendNormalDanmuke.bind(this)}>
              发送普通弹幕
            </button>
          </div>
          <div className="advance-danmuke">
            <input type="text" ref={this.advanceDanmukeRef} />
            <button onClick={this.sendAdvanceDanmuke.bind(this)}>
              发送高级弹幕
            </button>
          </div>
        </div>
